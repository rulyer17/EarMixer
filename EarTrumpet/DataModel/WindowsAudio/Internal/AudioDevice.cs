            ...
            set
            {
                if (value != _isMuted)
                {
                    try
                    {
                        ...
                        // Some capture-device drivers do not send an endpoint
                        // volume callback after SetMute.  Read their state back
                        // so the microphone control does not wait indefinitely.
                        if (Parent?.Kind == AudioDeviceKind.Recording.ToString())
                        {
                            var muted = _deviceVolume.GetMute() != 0;
                            if (muted != _isMuted)
                            {
                                _isMuted = muted;
                                _dispatcher.BeginInvoke((Action)(() => RaisePropertyChanged(nameof(IsMuted))));
                            }
                        }
                    }
                    catch (Exception ex) when (ex.Is(HRESULT.AUDCLNT_E_DEVICE_INVALIDATED))
